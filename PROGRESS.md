...
### 2026-07-11 (cont. 16) - retired profile-to-SigLIP Explore experiment

This historical experiment used `conditioning.profile_style_query(profile)` to embed
profile text for Explore cold start. It is no longer current: the uncalled helper and
its text-encoder claim were deleted. Current style-slate conditioning is owned by
`services/api/app/recsys/conditioning.py` and uses certain perceived item aesthetics,
so it remains deterministic while remote text encoding is degraded.
