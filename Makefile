...
.PHONY: data-export help install check-uv migrate dev dev-web dev-api up down logs stack stack-down stack-logs nuke fmt fmt-check lint typecheck test test-api repo-hygiene workflow-security ownership-inventory client-boundaries production-surfaces expo-hosting doctrine ci types m2-bakeoff m2-clean clean
...
workflow-security: ## Enforce the PR secret/untrusted-code workflow boundary
	ruby scripts/test_check_workflow_security.rb
	ruby scripts/check_workflow_security.rb
...
production-surfaces: ## Reject production review/debug routes and fixture assets
	python3 -m unittest discover -s scripts -p "test_check_production_surfaces.py"
	python3 scripts/check_production_surfaces.py
...
doctrine: repo-hygiene workflow-security ownership-inventory client-boundaries production-surfaces expo-hosting ## Run repo ownership, hygiene and doctrine gates
