...
the ``bodyshape`` extra + a GPU + the SAM License for the real model to run.
"""
...
def main(argv: list[str]) -> int:
    ...
    mesh = estimator.estimate(image)
    est = estimate_body(image, estimator)

    print(f"image:        {argv[1]}  ({image.width}x{image.height})")
    if mesh.vertices.shape[0]:
        measurements = mesh_to_measurements(mesh.vertices)
        print(f"measurements: {measurements}")
        print(f"ratios:       {ratios(measurements)}")
