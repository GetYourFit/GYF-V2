...
interface SavedItemCardProps { item: SavedItem; }
    ...
    >
      <div style={{ aspectRatio: "3/4", background: "#1a1a22", overflow: "hidden" }}>
        {src
          ...
          : <div style={{ display: "flex", alignItems: "center", justifyContent: "center", height: "100%", ...MONO, color: "#5a5a65" }}>No image</div>
        }
      </div>
...
interface OutfitCardProps { outfit: SavedOutfit; }
...
export function OutfitCard({ outfit }: OutfitCardProps) {
  ...
  return (
    ...
    >
      {/* 2-up preview grid */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1px", background: "#1a1a22", aspectRatio: "4/3" }}>
        {previews.length > 0 ? previews.map((p) => {
          ...
          return (
            <div key={p.item_id} style={{ overflow: "hidden", background: "#1a1a22" }}>
              {s
                // eslint-disable-next-line @next/next/no-img-element
                ? <img src={s} alt={p.title} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover", display: "block" }} />
                : null
              }
            </div>
          );
        }) : (
          <div style={{ gridColumn: "span 2", display: "flex", alignItems: "center", justifyContent: "center", height: "100%", ...MONO, color: "#5a5a65" }}>
            No items
          </div>
        )}
