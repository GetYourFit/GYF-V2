...
export function PostCard({ post, index = 0, onReact, onShared, onDressLikeMe }: PostCardProps) {
  ...
  async function react() {
    ...
    try {
      ...
      if (!ok) { setReacted(false); setCount((n) => Math.max(0, n - 1)); }
    } catch {
      setReacted(false);
  ...
  function share() {
    ...
    if (typeof navigator !== "undefined" && navigator.share) {
      void navigator.share({ title: "An outfit on GYF", text })
        .then(() => onShared?.("Look shared from your device."))
        .catch(() => {});
    } else if (typeof navigator !== "undefined" && navigator.clipboard) {
      void navigator.clipboard.writeText(text).then(() => onShared?.("Caption copied to your clipboard."));
    }
  }
  ...
  return (
    <motion.li
      ...
      transition={{ duration: 0.4, ease: LUX, delay: reduceMotion ? 0 : Math.min(index * 0.06, 0.36) }}
      style={{
        listStyle: "none",
        background: "#ffffff",
        borderRadius: "20px",
    ...
    >
      {/* Author row */}
      <div style={{ display: "flex", alignItems: "center", gap: "0.75rem", padding: "0.875rem 1rem 0.75rem" }}>
        <div
          style={{
        ...
        >
          <span style={{ fontFamily: "var(--font-body)", fontSize: "1rem", fontWeight: 700, color: "#5c5650" }}>
            {author.initial}
          </span>
        </div>
        <div style={{ minWidth: 0, flex: 1 }}>
          <p style={{ fontFamily: "var(--font-body)", fontSize: "0.9rem", fontWeight: 600, color: "#1c1a17", margin: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
            @{author.handle}
          </p>
          {post.region && (
            <p style={{ fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "#9a9490", margin: 0, marginTop: "1px" }}>
              {post.region}
            </p>
          )}
        </div>
      ...
      {heroImage && (
        <button
          ...
          style={{ position: "relative", aspectRatio: "4/5", width: "100%", overflow: "hidden", background: "#f4f1ec", border: "none", padding: 0, cursor: "pointer", display: "block" }}
        >
          ...
          {post.occasion && (
            <span style={{
              ...
              background: "rgba(255,255,255,0.90)", color: "#5c5650",
      ...
      {/* Reaction bar */}
      <div style={{ display: "flex", alignItems: "center", gap: "1.25rem", padding: "0.875rem 1rem 0.5rem" }}>
        <motion.button
          type="button"
      ...
      {post.caption && (
        <div style={{ padding: "0 1rem 1rem" }}>
          <p
            style={{
              fontFamily: "var(--font-body)", fontSize: "0.875rem", color: "#1c1a17", lineHeight: 1.5, margin: 0,
              display: "-webkit-box",
          ...
          {post.caption.length > 80 && (
            <button
              ...
              style={{ background: "none", border: "none", padding: 0, fontFamily: "var(--font-body)", fontSize: "0.8125rem", color: "#9a9490", cursor: "pointer", marginTop: "0.25rem" }}
            >
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", padding: "0 1rem 1rem" }}>
          {post.items.map((item) => (
            <span
              key={item.item_id}
              style={{
                fontFamily: "var(--font-body)", fontSize: "0.75rem", color: "#5c5650",
                background: "#f4f1ec", borderRadius: "999px", padding: "0.25rem 0.75rem",
                border: "1px solid rgba(0,0,0,0.08)",
              }}
            >
              {item.title}
