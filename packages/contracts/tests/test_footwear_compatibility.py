...
class TestFootwearClassification:
    ...
    def test_running_shoes(self):
        assert classify_footwear_style("Nike Running Shoes", "sneakers") == FootwearStyle.RUNNING_SHOES
        assert classify_footwear_style("Athletic Sports Shoes", "sneakers") == FootwearStyle.RUNNING_SHOES
    
    def test_canvas_sneakers(self):
        assert classify_footwear_style("Converse Canvas Sneakers", "sneakers") == FootwearStyle.CANVAS_SNEAKERS
        assert classify_footwear_style("Vans Classic Shoes", "sneakers") == FootwearStyle.CANVAS_SNEAKERS
    ...
    def test_fallback_classification(self):
        ...
        assert classify_footwear_style("Generic Sneakers", "sneakers") == FootwearStyle.CANVAS_SNEAKERS
...
class TestCompatibilityValidation:
    ...
    def test_valid_casual_combinations(self):
        ...
        assert is_compatible_outfit(
            FootwearStyle.CANVAS_SNEAKERS, "t_shirt", "jeans", "casual"
        ) is True
        
        # Wedges with casual dress
        assert is_compatible_outfit(
            FootwearStyle.WEDGES, "blouse", "skirt", "casual"
        ) is True
    
    def test_valid_formal_combinations(self):
        # Oxford shoes with business outfit
        assert is_compatible_outfit(
            FootwearStyle.OXFORD_SHOES, "shirt", "trousers", "business"
        ) is True
        
        # Pumps with formal dress
        assert is_compatible_outfit(
            FootwearStyle.PUMPS, "blouse", "skirt", "formal"
        ) is True
    
    def test_invalid_slipper_combinations(self):
        # House slippers should only work with loungewear
        assert is_compatible_outfit(
            FootwearStyle.HOUSE_SLIPPERS, "t_shirt", "shorts", "casual"
        ) is False
        
        assert is_compatible_outfit(
            FootwearStyle.HOUSE_SLIPPERS, "shirt", "trousers", "business"
        ) is False
        
        # But should work with loungewear
        assert is_compatible_outfit(
            FootwearStyle.HOUSE_SLIPPERS, "t_shirt", "shorts", "loungewear"
        ) is True
    
    def test_invalid_flip_flops_combinations(self):
        # Flip flops shouldn't work with formal clothing
        assert is_compatible_outfit(
            FootwearStyle.FLIP_FLOPS, "shirt", "trousers", "business"
        ) is False
        
        assert is_compatible_outfit(
            FootwearStyle.FLIP_FLOPS, "dress", None, "formal"
        ) is False
        
        # But should work with very casual outfits
        assert is_compatible_outfit(
            FootwearStyle.FLIP_FLOPS, "t_shirt", "shorts", "casual"
        ) is True
    
    def test_formal_shoes_with_shorts(self):
        # Formal shoes shouldn't work with shorts (coverage requirement)
        assert is_compatible_outfit(
            FootwearStyle.OXFORD_SHOES, "shirt", "shorts", "business"
        ) is False
    
    def test_traditional_compatibility(self):
        # Mojari should work with traditional Indian garments
        assert is_compatible_outfit(
            FootwearStyle.MOJARI, "kurta", "salwar", "festive"
        ) is True
        
        # But not with western casual
        assert is_compatible_outfit(
            FootwearStyle.MOJARI, "t_shirt", "jeans", "casual"
        ) is False
    
    def test_occasion_mismatch(self):
        # Running shoes at formal occasion
        assert is_compatible_outfit(
            FootwearStyle.RUNNING_SHOES, "shirt", "trousers", "formal"
        ) is False
        
        # Combat boots at wedding
        assert is_compatible_outfit(
            FootwearStyle.COMBAT_BOOTS, "dress", None, "wedding"
        ) is False
...
class TestOutfitValidation:
    ...
    def test_traditional_outfit(self):
        ...
        is_valid, reason = validate_footwear_outfit(
            "Mojari", "mojari", outfit_items, "festive"
        )
        assert is_valid is True
        assert "compatible" in reason.lower()
    ...
    def test_missing_top(self):
        ...
        is_valid, reason = validate_footwear_outfit(
            "Sneakers", "sneakers", outfit_items, "casual"
        )
        assert is_valid is False
...
class TestEdgeCases:
    ...
    def test_full_body_garments(self):
        ...
        assert is_compatible_outfit(
            FootwearStyle.WEDGES, "dress", None, "party"
        ) is True
        
        assert is_compatible_outfit(
            FootwearStyle.FLIP_FLOPS, "saree", None, "wedding"
        ) is False
    ...
    def test_unknown_occasion(self):
        ...
        ) is False  # Still fails due to garment incompatibility
