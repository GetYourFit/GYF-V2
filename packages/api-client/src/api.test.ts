...
describe("GyfApi", () => {
  ...
  it("reuses one generated event id when feedback retries", async () => {
    ...
    const headers = fetchSpy.mock.calls.map(([, init]) => new Headers(init?.headers));
    expect(headers[0].get("Idempotency-Key")).toBe(bodies[0].event_id);
    expect(headers[1].get("Idempotency-Key")).toBe(bodies[0].event_id);
    expect(headers[0].get("X-Request-ID")).toBe(headers[1].get("X-Request-ID"));
  });

  it.each([
    [401, "isUnauthorized"],
    [403, "isUnauthorized"],
    [429, "isQuotaExhausted"],
  ] as const)("preserves %s status mapping", async (status, getter) => {
    vi.stubGlobal("fetch", mockFetch({ status, body: { detail: "honest failure" } }));
    const error = await new GyfApi(() => "jwt", "http://api").getProfile().catch((e: unknown) => e);
