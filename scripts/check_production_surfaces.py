...
def source_failures(root: Path) -> list[str]:
    ...
    if "reviewSurfaceEnabled" not in expo_route or "local development" not in expo_route:
    ...
    if "notFound" not in next_route or 'process.env.NODE_ENV === "production"' not in next_route:
        failures.append("Next /design page is missing its production not-found gate")
    if 'pathname === "/design"' not in next_proxy:
        failures.append("Next production proxy does not reject /design before rendering")
...
def artifact_failures(dist: Path) -> list[str]:
    ...
    for path in dist.rglob("*"):
        ...
        for marker in FORBIDDEN_RELEASE_MARKERS:
            if marker in text:
                failures.append(f"{path.relative_to(dist)} contains review marker {marker!r}")
    return failures


def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--expo-dist", type=Path, help="also inspect a production Expo export")
    ...
    if failures:
        ...
        return 1
    print("production surface guard passed: review surfaces are local-only and contract-hidden")
    return 0
