...
| Frontend — Next.js `app/` | Preserve as a behavioural oracle only; Vercel is retired through the captain-authorised 2026-07-28 provider packet. | Do **not** delete `app/` before F13 parity. Retain the current fail-closed `vercel.json`, Vercel CORS origin and provider resources only until the landed-main Expo header/immutable-rollback proof passes; then remove the Vercel provider boundary through its Phase B operation. | Phase A replacement proof in progress; no external deletion before its PR lands. |
...
| Infra/workflows | Fix keepalive wording/ownership; update production-plan and Cuelinks workflows only when their gates open; review Flutter CI after coverage transfer. | Do not touch external deploy state or release branches from cleanup PRs. | Keepalive wording fixed (comment now describes the paid Virginia Starter's connection-pool/cache warm-keep, not a free-tier stopgap); production-plan/Cuelinks workflows and Flutter CI review remain planned. |
